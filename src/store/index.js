  ...
  mutations: {
    setRuns(state, runs) {
      ...
      for (var run in runs) {
        ...
        if (!state.runs.find((x) => x.id === currRun.id)) {
          // if the runs doesnt exist
          // cash in run
          state.bankedPoints = state.bankedPoints + currRun.distance / 100;
    ...
    setPokemonLevel(state, pokemonObj) {
      var pokemon = state.party.find(
        (x) => x.partyID === pokemonObj.pokemon.partyID
      );
    ...
    addPokemonToParty(state, pokemon) {
      var routeUsed = state.routesMetOn.find(
        (x) => x.routeName.toLowerCase() === pokemon.metOn.toLowerCase()
      );
    ...
    addRouteEncounter(state, obj) {
      var routeUsed = state.routesMetOn.find(
        (x) => x.routeName.toLowerCase() === obj.routeName.toLowerCase()
      );
    ...
    markAsDead(state, deadObj) {
      ...
      var partyIndex = state.party.findIndex(
        (x) => x.partyID === pokemon.partyID
      );
    ...
    markAsRevived(state, pokemon) {
      if (pokemon.level <= state.bankedPoints) {
        var partyIndex = state.party.findIndex(
          (x) => x.partyID === pokemon.partyID
        );
        var currPokemon = state.party[partyIndex];
        ...
        alert(
          `Not enough points! Go run ${pokemon.level -
            state.bankedPoints} hm more!`
        );
      }
    },
